...
var msw = {
    ...
    set: function (hash) {
        ...
        else if ('units' in hash && unitOptions.indexOf(hash.units.toLowerCase()) > 0) msw.units = hash.units.toLowerCase();
        return msw;
    },
    forecast: function (options) {
        ...
        if (!('units' in options)) options.units = msw.units;
        uri = msw.url({ 'spot_id': options.spotId , 'units': options.units });
