...
Forecast.prototype.where = function (query) {
    ...
    function getSequenceLength(startingIndex) {
        if (startingIndex < 0 || startingIndex >= trackPositions.length) return 0;
        if (trackPositions[startingIndex].included !== true) return 0;
        
        return 1 + getSequenceLength(startingIndex + 1);
    }

    var originalArray = this._data;
    //apply limits if any
    Object.keys(limits).forEach(function (key) {
        if (key !== 'minSequence') return;
        results = results.filter(function (entry, i) {
            var originalIndex = originalArray.indexOf(entry);
            console.log('oi: ' + originalIndex);
            console.log('seq: ' + getSequenceLength(originalIndex));
            return true;
        });
    });
    return results;
...
ForecastEntry.prototype.toString = function (options) {
    var i;
    var dateFormat = 'MMM D HH:mm  ';
    var output = (options.utc === true) ? this.localDatetime.utc().format(dateFormat) : this.localDatetime.format(dateFormat);
