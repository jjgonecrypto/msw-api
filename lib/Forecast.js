...
Forecast.prototype.where = function (query) {
    return _.filter(this._data, function (entry) {
        var condition = true;
        //breaking height
        if ('minBreakingHeight' in query) condition &= entry.swell.minBreakingHeight >= query.minBreakingHeight;
        if ('maxBreakingHeight' in query) condition &= entry.swell.maxBreakingHeight <= query.maxBreakingHeight;
        return condition;
    });
};
